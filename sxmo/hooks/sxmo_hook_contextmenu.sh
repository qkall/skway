#!/bin/sh
# configversion: 8ab90a06ea6971affb4f1854b3361e09
...
case "$WMCLASS" in
	...
	power)
		...
		CHOICES="
			...
			$icon_rol Switch WM          ^ 0 ^ sxmo_power.sh togglewm
			...
			case "$WMCLASS" in
				st|st-*)
					CHOICES="
						...
						$icon_itm Toggle Selmode  ^ 0 ^ sxmo_type.sh -M Ctrl -M Shift -k s
						$icon_cpy Copy complete   ^ 0 ^ sxmo_type.sh -M Ctrl -M Shift -k i
						$icon_cpy Copy selection  ^ 0 ^ sxmo_type.sh -M Ctrl -M Shift -k c
	...
	*)
		...
		CHOICES="
			...
			$icon_dir Files                                              ^ 0 ^ sxmo_terminal.sh --font=monospace:size=9 /usr/bin/yazi
			...
			$(
				if brightness="$(brightnessctl -d "white:flash" get)"; then
					...
					[ "$brightness" -gt 0 ] &&
						printf %b "$icon_ton" ||  printf %b "$icon_tof";
					printf %b "^ 0 ^ sxmo_flashtoggle.sh"
				fi
			)
			$icon_net Networks                                           ^ 0 ^ sxmo_networks.sh
			$icon_mus Audio                                              ^ 0 ^ sxmo_audio.sh
			$icon_phn Modem                                              ^ 0 ^ sxmo_appmenu.sh modem
			$icon_fnd Scale Display                                      ^ 0 ^ sxmo_scaledisplay.sh
			$icon_fnd Screenshot                                     	 ^ 0 ^ /usr/bin/sxmo_screenshot.sh
